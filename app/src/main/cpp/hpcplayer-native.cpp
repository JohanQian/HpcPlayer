...
// Helper to cast the jlong back to a HpcPlayerInterface pointer
// Since HpcPlayer inherits from HpcPlayerInterface, we can use the interface pointer.
inline HpcPlayerInterface* FromLong(jlong handle) {
    return reinterpret_cast<HpcPlayerInterface*>(handle);
}
...
Java_com_example_hpcplayer_HpcNativePlayer_nativeInit(JNIEnv* env, jobject thiz) {
    // Instantiate HpcPlayer but return as HpcPlayerInterface*
    return reinterpret_cast<jlong>(static_cast<HpcPlayerInterface*>(new HpcPlayer()));
}
